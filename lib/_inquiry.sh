...
get_link_git() {
  ...
  print_banner
  printf "${WHITE} 💻 Insira o link do GITHUB do seu Chatbot oficial que deseja instalar:${GRAY_LIGHT}"
  printf "\n\n"
  read -p "> " link_git
}
...
inquiry_options() {
  ...
  print_banner
  printf "${WHITE} 💻 Bem vindo(a) ao Gerenciador Chatbot oficial - Atendimento sem limites, selecione abaixo a proxima ação!${GRAY_LIGHT}"
  printf "\n\n"
  printf "   [0] Instalar Chatbot oficial\n"
  printf "   [1] Atualizar Chatbot oficial\n"
  printf "   [2] Deletar Chatbot oficial\n"
  printf "   [3] Bloquear Chatbot oficial\n"
  printf "   [4] Desbloquear Chatbot oficial\n"
  printf "   [5] Alter. dominio Chatbot oficial\n"
  printf "\n"
  read -p "> " option
